...
ProveKit is the open-source universal agent client: test, debug, and evaluate any AI
agent (LLM API, MCP server, HTTP agent, A2A agent) with no SDK. Monorepo:

- `backend/` — FastAPI over SQLAlchemy; local/dev defaults to SQLite (`provekit.db`), while
  ...
  (`backend/provekit/migrations/`) so an installed wheel can migrate its own database;
  keep them there. Python package is `provekit` (installable;
  provides the `provekit` CLI). Run the API with `uvicorn provekit.main:app --port 8100`.
...
- Backend: keep modules small and single-purpose. Providers live in
  `provekit/services/providers/` and yield the unified event schema in
...
- Secrets: masked in API responses (`services/masking.py`) and encrypted at rest
  (`services/sealing.py`). Never write a credential into an `.provekit` file
...
Spec in `docs/FILE_FORMAT.md`; runnable examples in `examples/.provekit/`. The `provekit`
CLI (`backend/provekit/cli.py`) runs them headless for CI.
