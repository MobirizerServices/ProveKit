...
export default function TracesPage() {
  ...
  const load = () => api.runs().then(setTraces).catch(() => {});
  useEffect(() => {
    load();
    const t = setInterval(load, 5000);   // live-ish: new runs stream in
    ...
    <>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 20px 80px" }}>
        ...
        <p className="muted" style={{ margin: "0 0 20px", fontSize: 13.5 }}>
          Every run your agent makes, captured via the tracing decorator — the model, the
          input, the output, and timing.
        </p>
        ...
        {traces.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 16 }}>
            <div style={{ ...panel, padding: 0, maxHeight: "72vh", overflowY: "auto" }}>
              {traces.map((t) => (
              ...
              {!detail ? (
                ...
                <>
                  ...
                  <div className="muted" style={{ fontSize: 12, margin: "2px 0 14px" }}>
                    ...
                    {detail.request?.provider && ` · ${detail.request.provider}`} · {detail.status} · {detail.duration_ms}ms
                  </div>
